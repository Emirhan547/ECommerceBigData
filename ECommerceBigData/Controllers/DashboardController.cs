...
public class DashboardController : Controller
{
    ...
    [HttpGet]
    public async Task<IActionResult> Index(DateTime? from, DateTime? to, string? country, string? city, string? category, string? segment)
    {
        var model = await BuildDashboardModelAsync(from, to, country, city, category, segment);
        model.CurrentPage = "Genel Bakış";
        return View(model);
    }

    [HttpGet]
    public async Task<IActionResult> Sales(DateTime? from, DateTime? to, string? country, string? city, string? category, string? segment)
    {
        var model = await BuildDashboardModelAsync(from, to, country, city, category, segment);
        model.CurrentPage = "Satış";
        return View(model);
    }

    [HttpGet]
    public async Task<IActionResult> Customers(DateTime? from, DateTime? to, string? country, string? city, string? category, string? segment)
    {
        var model = await BuildDashboardModelAsync(from, to, country, city, category, segment);
        model.CurrentPage = "Müşteri";
        return View(model);
    }

    [HttpGet]
    public async Task<IActionResult> Products(DateTime? from, DateTime? to, string? country, string? city, string? category, string? segment)
    {
        var model = await BuildDashboardModelAsync(from, to, country, city, category, segment);
        model.CurrentPage = "Ürün";
        return View(model);
    }

    [HttpGet]
    public async Task<IActionResult> Geography(DateTime? from, DateTime? to, string? country, string? city, string? category, string? segment)
    {
        var model = await BuildDashboardModelAsync(from, to, country, city, category, segment);
        model.CurrentPage = "Coğrafi";
        return View(model);
    }

    [HttpGet]
    public async Task<IActionResult> Forecast(DateTime? from, DateTime? to, string? country, string? city, string? category, string? segment)
    {
        var model = await BuildDashboardModelAsync(from, to, country, city, category, segment);
        model.ForecastResult = await _salesForecastService.BuildForecastAsync(model.Filters);
        model.CurrentPage = "Forecast(ML)";
        return View(model);
    }
        ...
        string? segment)
    {
        ...
        var segmentsTask = _dashboardRepository.GetSegmentFilterOptionsAsync(filters);
    ...
    {
        return CalculateGrowthRate((decimal)current, previous);
    }
}